...
class SISEnrollmentController extends Controller {
  ...
  public function indexAction() {
    ...
    return $this->render('KulaK12StudentBundle:SISEnrollment:statuses.html.twig', array('statuses' => $statuses));
  }
  ...
  public function enrollmentsAction($student_status_id) {
    ...
    return $this->render('KulaK12StudentBundle:SISEnrollment:enrollments.html.twig', array('enrollments' => $enrollments, 'status' => $status));
  }
  ...
  public function activityAction($student_enrollment_id) {
    ...
    return $this->render('KulaK12StudentBundle:SISEnrollment:activity.html.twig', array('activity' => $activity, 'enrollment' => $enrollment, 'status' => $status));
  }
      ...
      if ($status['STATUS'] == '') {
        ...
        if ($enrollmentInfo = $this->form('edit', 'K12.Student.Enrollment')) {
          ...
          if ($enrollmentPoster AND $statusPoster) {
            ...
            return $this->forward('sis_K12_student_enrollment_statuses', array('record_type' => 'SIS.K12.Student', 'record_id' => $status['STUDENT_ID']), array('record_type' => 'SIS.K12.Student', 'record_id' => $status['STUDENT_ID']));
          } else {
        ...
        return $this->render('KulaK12StudentBundle:SISEnrollment:inactivate.html.twig', array('status' => $status, 'enrollment' => $enrollment));
        ...
        return $this->render('KulaK12StudentBundle:SISEnrollment:activate.html.twig', array('status' => $status));
      }
  ...
  public function editAction() {
    ...
    if ($this->record->getSelectedRecordID()) {
      ...
      if ($activity_post = $this->form('add', 'K12.Student.Enrollment.Activity', 'new')) {
        ...
        if ($activity_poster) {
          $transaction->commit();
          return $this->forward('sis_K12_student_enrollment_statuses', array('record_type' => 'SIS.K12.Student.Status', 'record_id' => $this->record->getSelectedRecordID()), array('record_type' => 'SIS.K12.Student.Status', 'record_id' => $this->record->getSelectedRecordID()));
        } else {
          $transaction->rollback();
    ...
    return $this->render('KulaK12StudentBundle:SISEnrollment:edit.html.twig', array('status' => $status, 'effective_date' => $effective_date));
