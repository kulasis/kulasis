...
class CoreInformationController extends Controller {
  ...
  public function basicAction() {
    ...
    return $this->render('KulaK12StudentBundle:CoreInformation:basic.html.twig', array('status' => $status, 'addresses' => $addresses, 'phones' => $phones, 'emails' => $emails));
  }
  ...
  public function demographicAction() {
    ...
    return $this->render('KulaK12StudentBundle:CoreInformation:demographic.html.twig', array('student' => $student));
  }
  ...
  public function other_infoAction() {
    ...
    return $this->render('KulaK12StudentBundle:CoreInformation:other_info.html.twig', array('student' => $student, 'student_status' => $student_status));
  }
  ...
  public function addAction() {
    ...
    if ($this->form('add', 'K12.Student.Status', 'new', 'K12.Student.Status.StudentID')) {
      $student_id = $this->createStudent();
      return $this->forward('Core_K12_Student_Student_Basic', array('record_type' => 'Core.K12.Student', 'record_id' => $student_id), array('record_type' => 'Core.K12.Student', 'record_id' => $student_id));
    }
    ...
    return $this->render('KulaK12StudentBundle:CoreInformation:add.html.twig', array('constituents' => $constituents, 'defaults' => $defaults));
  }
  
  public function add_constituentAction() {
    $this->authorize();
    $this->formAction('Core_K12_Student_Student_Create_Constituent');
    ...
    return $this->render('KulaK12StudentBundle:CoreInformation:add_constituent.html.twig', array('defaults' => $defaults));
  }
  ...
  private function createStudent() {
    ...
    $studentInfo = array_merge($studentInfo, $this->form('add', 'K12.Student.Status', 'new'));
    
    $student_id = $this->get('kula.K12.student')->addStudent(
      $this->form('add', 'K12.Student.Status', 'new', 'K12.Student.Status.StudentID'), 
    ...
    $enrollmentInfo = array_merge($enrollmentInfo, $this->form('add', 'K12.Student.Status', 'new'));
    $enrollment = $this->get('kula.K12.student')->enrollStudent($enrollmentInfo);
    
    $this->get('kula.K12.billing.constituent')->determineTuitionRate($enrollment['student_status']);
    
    $this->get('kula.K12.billing.student')->checkMandatoryTransactions($enrollment['student_status']);
  ...
  public function index_teacherAction() {
    $this->authorize();
    return $this->render('KulaK12StudentBundle:CoreInformation:index_teacher.html.twig');
    
  }
  
  public function chooserAction() {
    $this->authorize();
    $data = $this->chooser('Core.K12.Student')->createChooserMenu($this->request->query->get('q'));
