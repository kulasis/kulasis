...
class CoreBillingReceiptReportController extends ReportController {
  ...
  public function indexAction() {
    ...
    return $this->render('KulaCoreBillingBundle:CoreBillingReceiptReport:reports_billingreceipt.html.twig', array('student_payments' => $student_payments));
  }
  ...
  {  
    ...
    $this->pdf = new \Kula\Core\Bundle\BillingBundle\Report\BillingReceiptReport("P");
    ...
    $result = $this->db()->db_select('CONS_CONSTITUENT', 'stucon')
      ...
      ->leftJoin('STUD_STUDENT', 'student', 'student.STUDENT_ID = stucon.CONSTITUENT_ID')
      ...
      ->fields('billaddr', array('THOROUGHFARE' => 'bill_ADDRESS', 'ADMINISTRATIVE_AREA' => 'bill_STATE', 'LOCALITY' => 'bill_CITY', 'POSTAL_CODE' => 'bill_ZIPCODE', 'COUNTRY' => 'bill_COUNTRY'))
      ->leftJoin('CONS_ADDRESS', 'mailaddr', 'mailaddr.ADDRESS_ID = stucon.MAILING_ADDRESS_ID AND mailaddr.UNDELIVERABLE = 0')
      ->fields('mailaddr', array('THOROUGHFARE' => 'mail_ADDRESS', 'ADMINISTRATIVE_AREA' => 'mail_STATE', 'LOCALITY' => 'mail_CITY', 'POSTAL_CODE' => 'mail_ZIPCODE', 'COUNTRY' => 'mail_COUNTRY'))
      ...
      ->fields('residenceaddr', array('THOROUGHFARE' => 'residence_ADDRESS', 'ADMINISTRATIVE_AREA' => 'residence_STATE', 'LOCALITY' => 'residence_CITY', 'POSTAL_CODE' => 'residence_ZIPCODE', 'COUNTRY' => 'residence_COUNTRY'));
    $org_term_ids = $this->focus->getOrganizationTermIDs();
    if ($this->focus->getTermID() != '' AND isset($org_term_ids) AND count($org_term_ids) > 0) {
        $result = $result->leftJoin('STUD_STUDENT_STATUS', 'status', 'status.STUDENT_ID = student.STUDENT_ID AND status.ORGANIZATION_TERM_ID IN ('.implode(', ', $org_term_ids).')')
    ...
    $result = $result
      ...
      ->orderBy('stucon.CONSTITUENT_ID', 'ASC')
      ->execute();
    ...
    while ($row = $result->fetch()) {
      ...
      if ($row['bill_ADDRESS']) {
        ...
        $billing_addresses_result = $this->db()->db_select('CONS_ADDRESS', 'address')
          ...
          ->condition('CONSTITUENT_ID', $row['CONSTITUENT_ID'])
          ->condition('ACTIVE', 1)
        ...
        while ($billing_addresses_row = $billing_addresses_result->fetch()) {
          ...
          $this->createStatement($row['CONSTITUENT_ID'], $row);
        }
      }
