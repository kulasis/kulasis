...
class SISHomeController extends Controller {
  ...
  public function homeAction() {
    ...
    $enrolled = $this->db()->db_select('STUD_STUDENT_STATUS')
      ...
      ->leftJoin('CORE_LOOKUP_VALUES', 'value', "value.CODE = STUD_STUDENT_STATUS.GRADE AND value.LOOKUP_TABLE_ID = (SELECT LOOKUP_TABLE_ID FROM CORE_LOOKUP_TABLES WHERE LOOKUP_TABLE_NAME = 'HEd.Student.Enrollment.Grade')")
    ...
    $total_credits = $this->db()->db_select('STUD_STUDENT_CLASSES', 'classes')
      ...
      ->leftJoin('CORE_LOOKUP_VALUES', 'value', 'value.CODE = stustatus.GRADE')
      ->fields('value', array('DESCRIPTION' => 'GRADE'))
      ->condition('ORGANIZATION_TERM_ID', $this->focus->getOrganizationTermIDs())
    ...
    $full_fte = $this->db()->db_select('STUD_STUDENT_CLASSES', 'classes')
      ->expression('SUM(classes.CREDITS_ATTEMPTED)', 'total_credits')
      ->join('STUD_STUDENT_STATUS', 'stustatus', 'stustatus.STUDENT_STATUS_ID = classes.STUDENT_STATUS_ID')
      ->fields('stustatus', array('LEVEL', 'FTE'))
    ...
    $enrolled_list = $this->db()->db_select('STUD_STUDENT_STATUS', 'status')
      ->fields('status', array('LEVEL', 'ENTER_DATE', 'STATUS', 'LEAVE_DATE'))
      ...
      ->leftJoin('CORE_LOOKUP_VALUES', 'value', 'value.CODE = status.GRADE')
      ->fields('value', array('DESCRIPTION' => 'GRADE'))
      ->leftJoin('CORE_LOOKUP_VALUES', 'entercode_value', "entercode_value.CODE = status.ENTER_CODE AND entercode_value.LOOKUP_TABLE_ID = (SELECT LOOKUP_TABLE_ID FROM CORE_LOOKUP_TABLES WHERE LOOKUP_TABLE_NAME = 'HEd.Student.Enrollment.EnterCode')")
      ->fields('entercode_value', array('DESCRIPTION' => 'ENTER_CODE'))
      ->leftJoin('CORE_LOOKUP_VALUES', 'leavecode_value', "leavecode_value.CODE = status.LEAVE_CODE AND leavecode_value.LOOKUP_TABLE_ID = (SELECT LOOKUP_TABLE_ID FROM CORE_LOOKUP_TABLES WHERE LOOKUP_TABLE_NAME = 'HEd.Student.Enrollment.LeaveCode')")
