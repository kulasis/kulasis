...
class CoreStudentRecord extends Record {
  
  public function getSelectedRecordBarTemplate() {
    return 'KulaHEdStudentBundle::CoreRecord/selected_record_student.html.twig';
  }
  
  public function getRecordBarTemplate() {
    return 'KulaHEdStudentBundle::CoreRecord/record_student.html.twig';
  }
  
  public function getFromDifferentType($record_type, $record_id) {
    if ($record_type == 'Core.HEd.Student.Status' OR $record_type == 'Core.K12.Student.Status') {
  ...
  public function get($record_id) {
    ...
    $result = $this->db()->db_select('STUD_STUDENT', 'stu')
      ...
      ->join('CONS_CONSTITUENT', 'constituent', 'constituent.CONSTITUENT_ID = stu.STUDENT_ID')
      ->fields('constituent', array('CONSTITUENT_ID', 'PERMANENT_NUMBER', 'LAST_NAME', 'FIRST_NAME', 'MIDDLE_NAME', 'IDENTIFIED_GENDER', 'GENDER', 'RACE', 'MAIDEN_NAME', 'PREFERRED_NAME', 'DEVELOPMENT_NUMBER'));
