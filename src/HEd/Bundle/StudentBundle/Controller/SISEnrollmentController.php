  ...
  public function editAction() {
    ...
    if ($this->record->getSelectedRecordID()) {
      ...
      if ($activity_post = $this->form('add', 'HEd.Student.Enrollment.Activity', 'new')) {
        ...
        if ($activity_poster) {
          $transaction->commit();
          return $this->forward('sis_HEd_student_enrollment_statuses', array('record_type' => 'SIS.HEd.Student.Status', 'record_id' => $this->record->getSelectedRecordID()), array('record_type' => 'SIS.HEd.Student.Status', 'record_id' => $this->record->getSelectedRecordID()));
        } else {
          $transaction->rollback();
    ...
    return $this->render('KulaHEdStudentBundle:SISEnrollment:edit.html.twig', array('status' => $status, 'effective_date' => $effective_date));
  }
