...
class InformationController extends Controller {
	...
	public function basicAction() {
		...
		if ($this->record->getSelectedRecordID()) {
			...
			$status = $this->db()->select('STUD_STUDENT_STATUS', 'stustatus')
				...
				->left_join('STUD_STUDENT_DEGREES', 'studegree', array('STUDENT_DEGREE_ID', 'DEGREE_ID', 'EXPECTED_COMPLETION_TERM_ID', 'GRADUATION_DATE'), 'stustatus.SEEKING_DEGREE_1_ID = studegree.STUDENT_DEGREE_ID')
				->left_join('STUD_STUDENT_DEGREES_MAJORS', 'stumajor', array('MAJOR_ID'), 'stumajor.STUDENT_DEGREE_ID = studegree.STUDENT_DEGREE_ID')
				->left_join('STUD_STUDENT_DEGREES_MINORS', 'stuminor', array('MINOR_ID'), 'stuminor.STUDENT_DEGREE_ID = studegree.STUDENT_DEGREE_ID')
				->left_join('STUD_STUDENT_DEGREES_CONCENTRATIONS', 'stuconcentration', array('CONCENTRATION_ID'), 'stuconcentration.STUDENT_DEGREE_ID = studegree.STUDENT_DEGREE_ID')
				->predicate('STUDENT_STATUS_ID', $this->record->getSelectedRecord()['STUDENT_STATUS_ID'])
				->execute()->fetch();

			}
			
			$address_result = $this->db()->select('CONS_ADDRESS', null)
				->fields(null, array('ADDRESS_ID', 'ADDRESS_TYPE', 'EFFECTIVE_DATE', 'ADDRESS', 'CITY', 'STATE', 'ZIPCODE', 'COUNTRY'))
			...
			$phone_result = $this->db()->select('CONS_PHONE', null)
		...
		return $this->render('KulaHEdStudentBundle:Information:basic.html.twig', array('status' => $status, 'addresses' => $addresses, 'phones' => $phones, 'emails' => $emails));
	}
	...
	public function demographicAction() {
		...
		$student = $this->db()->select('STUD_STUDENT', null)
			->join('CONS_CONSTITUENT', 'constituent', null, 'constituent.CONSTITUENT_ID = STUD_STUDENT.STUDENT_ID')
			->predicate('STUDENT_ID', $this->record->getSelectedRecord()['STUDENT_ID'])
			->execute()->fetch();
	...
	public function enrollmentAction() {
		...
		if ($this->record->getSelectedRecordID()) {
			...
			if (isset($this->request->request->get('add')['STUD_STUDENT_ENROLLMENT_ACTIVITY']['new'])) {
				...
				$enrollment = $this->db()->select('STUD_STUDENT_ENROLLMENT')
				...
				$activity_exists = $this->db()->select('STUD_STUDENT_ENROLLMENT_ACTIVITY')
					->fields(null, array('EFFECTIVE_DATE', 'ENROLLMENT_ACTIVITY_ID'))
					->predicate('ENROLLMENT_ID', $enrollment['ENROLLMENT_ID'])
			...
			$status = $this->db()->select('STUD_STUDENT_STATUS')
				->predicate('STUDENT_STATUS_ID', $this->record->getSelectedRecordID())
				->execute()->fetch();
	...
	private function _create_student($constituent_id = false) {
		...
		$constituent_id = $constituent_poster->getResultForTable('insert', 'CONS_CONSTITUENT')['new'];
			...
			$student_id_result = $connect->select('STUD_STUDENT')->fields(null, array('STUDENT_ID'))->predicate('STUDENT_ID', $constituent_id)->execute()->fetch();
		...
		if (isset($degree_information['STUD_STUDENT_DEGREES']) AND $degree_information['STUD_STUDENT_DEGREES']['new']['DEGREE_ID'] != '') {
			...
			$this->db()->select('CORE_TERM', 'term')
				->fields('term', 'END_DATE')
				->predicate('TERM_ID', $degree_information['new']['TERM_ID']['value'])
				->execute()->fetch();
			...
			$last_degree = $this->db()->select('STUD_STUDENT_DEGREES')
		...
		$enter_term = $this->db()->select('STUD_STUDENT_STATUS', 'status')
			->fields('status', array('ENTER_TERM_ID', 'ADMISSIONS_COUNSELOR_ID', 'COHORT', 'ADVISOR_ID'))
			->predicate('STUDENT_ID', $constituent_id)
			->predicate('LEVEL', $student_status_addition['new']['LEVEL'])
			->order_by('ENTER_DATE', 'ASC')
			->execute()->fetch();
		...
		$last_status_info = $this->db()->select('STUD_STUDENT_STATUS', 'status')
