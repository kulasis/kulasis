...
class CoreStudentTranscriptReportController extends ReportController {
  ...
  {  
    ...
    $last_name_filter = (isset($form['lastNameFilter']) AND $form['lastNameFilter'] != '') ? $form['lastNameFilter'] : '';
    $transcripts_with_course_history = (isset($form['generateStudentsWithCourseHistory']) AND $form['generateStudentsWithCourseHistory'] == 'Y') ? 'Y' : 'N';
    ...
    if ($last_name_filter != "") {
      $result = $result->condition('stucon.LAST_NAME', $last_name_filter, 'LIKE');
    }
    ...
    while ($row = $result->fetch()) {
      ...
      // Only if transcipt has course history data
      if ($transcripts_with_course_history == 'N' OR ($transcripts_with_course_history == 'Y' AND $this->service->hasTranscriptData() === true)) {
      ...
      } // end if on if course history data exists
