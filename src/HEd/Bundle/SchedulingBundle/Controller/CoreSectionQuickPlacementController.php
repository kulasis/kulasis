...
class CoreSectionQuickPlacementController extends Controller {
  ...
  public function indexAction() {
    ...
    if ($edit AND isset($edit['HEd.Student.Class'])) {
    ...
    if ($add AND isset($add['HEd.Student.Class'])) {
      
      $schedule_service = $this->get('kula.HEd.scheduling.schedule');
      foreach($add['HEd.Student.Class'] as $stustatusid => $section_id) {
        if ($section_id != '')
    ...
    if ($non AND isset($non['HEd.Section']['search_for_section']['HEd.Section.CourseID']['value'])) {
      $course_to_search = $non['HEd.Section']['search_for_section']['HEd.Section.CourseID']['value'];
    }
    ...
    return $this->render('KulaHEdSchedulingBundle:CoreSectionQuickPlacement:index.html.twig', array('course_to_search' => $course_to_search, 'students' => $students, 'students_with_course' => $students_with_course, 'sections' => $sections));
  }
}